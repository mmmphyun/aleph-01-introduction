...
const POSITIONS = {
  base: [
    { x: 50.0, y: 95.0 }, // GK (중앙)
    { x: 15.0, y: 78.0 }, // LB  <-> RB (x: 85.0)
    { x: 36.0, y: 80.0 }, // LCB <-> RCB (x: 64.0)
    { x: 64.0, y: 80.0 }, // RCB <-> LCB (x: 36.0)
    { x: 85.0, y: 78.0 }, // RB  <-> LB (x: 15.0) [활성 - 풀백]
    { x: 32.0, y: 56.0 }, // LCM <-> RCM/CM (x: 68.0)
    { x: 50.0, y: 68.0 }, // Regista [6번 앵커 - 중앙]
    { x: 68.0, y: 56.0 }, // CM  <-> LCM (x: 32.0) [활성 - 중앙 미드]
    { x: 18.0, y: 32.0 }, // LW  <-> RW (x: 82.0)
    { x: 50.0, y: 24.0 }, // ST  [스트라이커 - 중앙]
    { x: 82.0, y: 32.0 }, // RW  <-> LW (x: 18.0)
  ],
  striker: [
  ...
  fullback: [
    // 3) 오버래핑 상황: 하프라인 완벽 1자 3백(y:50), 전진한 레지스타(y:38), LCM 박스 지원, RB 오버래핑
    { x: 50.0, y: 78.0 }, // GK (하프라인 뒤 스위퍼)
    { x: 26.0, y: 50.0 }, // LB  -> [3백 좌측, 하프라인 완벽 1자]
    { x: 48.0, y: 50.0 }, // LCB -> [3백 중앙, 하프라인 완벽 1자]
    { x: 70.0, y: 50.0 }, // RCB -> [3백 우측, 하프라인 완벽 1자]
    { x: 90.0, y: 10.0 }, // RB  -> [우측 코너 플래그 끝까지 오버래핑!]
    { x: 28.0, y: 22.0 }, // LCM -> [박스 전면 좌측 2선 지원]
    { x: 52.0, y: 38.0 }, // Regista -> [하프라인 3백(y:50)과 미드(y:24) 사이를 잇는 전진 배치 & 조율]
    { x: 74.0, y: 26.0 }, // CM  -> [오버래핑한 RB 대각선 뒤에서 리턴 연계 지원]
    { x: 24.0, y: 14.0 }, // LW  (파 포스트 쇄도)
    { x: 46.0, y: 10.0 }, // ST  -> [박스 중앙 침투]
    { x: 66.0, y: 12.0 }, // RW  -> [박스 안 좁혀 투톱 형성]
  ],
};
