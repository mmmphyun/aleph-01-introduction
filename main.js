...
const POSITIONS = {
  ...
  fullback: [
    // 3) 오버래핑 상황: 수비 3백(하프라인), RB 오버래핑, RW가 박스로 좁혀 ST와 투톱 형성, Regista 중원 지배
    { x: 50.0, y: 82.0 }, // GK
    { x: 30.0, y: 52.0 }, // LB  -> [3백 좌측 스토퍼, 하프라인]
    { x: 50.0, y: 50.0 }, // LCB -> [3백 중앙, 하프라인]
    { x: 70.0, y: 50.0 }, // RCB -> [3백 우측 커버, 하프라인]
    { x: 90.0, y: 10.0 }, // RB  -> [우측 코너 플래그 끝까지 오버래핑!]
    { x: 34.0, y: 36.0 }, // LCM (좌측 밸런스 유지)
    { x: 48.0, y: 48.0 }, // Regista -> [중원 중앙의 넓은 빈 공간 완벽 점유 & 전환 패스 기점]
    { x: 74.0, y: 28.0 }, // CM  -> [오버래핑한 RB 대각선 뒤에서 리턴/연계 지원]
    { x: 26.0, y: 16.0 }, // LW  (파 포스트 쇄도)
    { x: 46.0, y: 10.0 }, // ST  -> [박스 중앙 침투]
    { x: 66.0, y: 12.0 }, // RW  -> [박스 안으로 완전히 좁혀 ST와 투톱 형성!]
  ],
};
